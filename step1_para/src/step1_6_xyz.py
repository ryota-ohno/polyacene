...
def init_process(args):
    ...
    def get_init_para_csv(auto_dir,monomer_name):
        ...
        init_para_list = []
        A1 = 0; A2 = 0
        theta_list = [22.5,25.5,26,27,27.5]#[5,10,15,20,25,30,35,40,45]#list(range(0,95,5)) ##+ [22.5,23.0,23.5,24.0,24.5] + [25.5,26.0,26.5,27.0,27.5]##全体を掃く＋細かいところ
    ...
    if not os.path.exists(auto_csv_path):        
        df_E_init = pd.DataFrame(columns = ['a','b','theta','E','E_p','E_t1','E_t2','machine_type','status','file_name'])##隣接8分子
    else:
...
def main_process(args):
    ...
    auto_csv_path = os.path.join(auto_dir,'step1.csv')
    if not os.path.exists(auto_csv_path):        
        df_E = pd.DataFrame(columns = ['a','b','theta','E','E_p','E_t1','E_t2','machine_type','status','file_name'])##いじる
...
def listen(auto_dir,monomer_name,num_nodes,isTest):##args自体を引数に取るか中身をばらして取るかの違い
    ...
    for idx,row in zip(df_queue.index,df_queue.values):
        ...
        if len(E_list)!=3:##get Eの長さは計算した分子の数
            ...
            Et1=float(E_list[0]);Et2=float(E_list[1]);Ep=float(E_list[2])##8分子に向けてep1,ep2作成　ep1:b ep2:a
            E = 2*Et1+2*Et2+2*Ep##エネルギーの値も変える
    ...
    while True:
        ...
        for a in [a_init_prev-0.1,a_init_prev,a_init_prev+0.1]:
            for b in [b_init_prev-0.1,b_init_prev,b_init_prev+0.1]:
                ...
                df_val_ab = df_val[(df_val['a']==a)&(df_val['b']==b)&(df_val['theta']==theta)&(df_val['Rt']==Rt)&(df_val['Rp']==Rp)&(df_val['status']=='Done')]
                if len(df_val_ab)==0:
