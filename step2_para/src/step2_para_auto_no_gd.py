...
from make_step2_auto_no_gd import exec_gjf
...
def listen(log_filepath_1,log_filepath_2):##args自体を引数に取るか中身をばらして取るかの違い    
    if not(os.path.exists(log_filepath_1)):
        isOver = False
    if not(os.path.exists(log_filepath_2)):
        isOver =False
...
def end_process(args):
    ...
    result_txt=os.path.join(auto_dir, 'step2_result.txt')
