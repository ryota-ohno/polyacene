...
##step3 精密化　tetracene pentaceneパターン　a,θ,czを最適化
import os
...
def main_process(args):
    ...
    if not os.path.exists(auto_csv_path):       
        df_E = pd.DataFrame(columns = ['cx','cy','cz','a','b','theta','Rt','Rp','E','E_i01','E_i02','E_ip1','E_ip2','E_ip3','E_ip4','E_it1','E_it2','E_it3','E_it4','machine_type','status','file_name'])##いじる
...
def listen(args):
    ...
    #### TODO
    fixed_param_keys = ['Rt','Rp','a','b','theta']
    ...
    if isAvailable:
        ...
        if len(params_dict)!=0:#終わりがまだ見えないなら
            ...
            if not(alreadyCalculated):
                #### TODO
                file_name = exec_gjf(auto_dir, monomer_name,params_dict, machine_type,isTest=isTest)##paramsdictとか
        ...
        for cx in [cx_init_prev-0.1,cx_init_prev,cx_init_prev+0.1]:
            for cy in [cy_init_prev-0.1,cy_init_prev,cy_init_prev+0.1]:
                for cz in [cz_init_prev-0.1,cz_init_prev,cz_init_prev+0.1]:
                    ...
                    cx = np.round(cx,1);theta = np.round(theta,1);cz= np.round(cz,1)
                    df_val_params = df_val[
                    (df_val['cx']==cx)&(df_val['cy']==cy)&(df_val['cz']==cz)&(df_val['Rt']==Rt)&(df_val['Rp']==Rp)
                    &(df_val['status']=='Done')
                       ]
